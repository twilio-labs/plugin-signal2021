...
export function ScrollableItemList({
  ...
  const [slicedEntries, offset] = useMemo(() => {
    ...
    let offset = 0;
    if (
      activeIdxPerLine > numberOfRowsShown / 2 &&
      numberOfRowsShown !== allNeededRows
    ) {
      offset = growsFromBottom
        ? Math.round((activeIdxPerLine - numberOfRowsShown) / rowsPerLine) + 2
        : Math.round(
            (activeIdxPerLine - Math.floor(numberOfRowsShown / 2)) / rowsPerLine
          );
    }
    ...
    if (growsFromBottom) slicedEntries.reverse();
  ...
  return (
    ...
    >
      ...
      <Box
        ref={ref}
        flexGrow={1}
        flexDirection={growsFromBottom ? 'column-reverse' : 'column'}
      >
        {slicedEntries}
      </Box>
